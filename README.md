...
| [`AGENT_ARCHITECTURE.md`](docs/AGENT_ARCHITECTURE.md) | Defines the proposed Phase 6 local-agent runtime and safety boundaries | When the agent runtime or provider direction changes |
...
The next authorized work is **Phase 6 — Validation and Hardening**: correct the foundation findings, then evaluate a locally orchestrated Claude Code agent in shadow mode using the operator's existing subscription, without adding a usage-metered model API to the initial pilot (DEC-045). Scoring, approvals, publication, Gmail handoff, and delivery state remain deterministic HORUS responsibilities. The agent architecture is documented in [`docs/AGENT_ARCHITECTURE.md`](docs/AGENT_ARCHITECTURE.md); no agent runtime is implemented yet.
