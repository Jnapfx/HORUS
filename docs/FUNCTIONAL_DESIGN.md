...
flowchart LR
  ...
  H -- "Return to edit" --> G
  H -- "Explicit approval" --> I["Create Gmail draft"]
  I --> J["Operator sends and declares status"]
  J --> K["Track next action"]
```

Only the two decision nodes lead to publication or Gmail-draft creation. HORUS has no transition that sends email.
...
| Outreach | `draft`, `awaiting_approval`, `gmail_draft_created`, `sent_declared`, `not_sent` | HORUS can create a Gmail draft only after explicit approval; it never sends. |
...
| 9. Approve and create Gmail draft | Explicitly approve the final outreach. | Create a Gmail draft using draft-only permission; store its created status and present the next manual action: open and send in Gmail. | Draft-creation failure is retryable and does not mark the message sent. HORUS has no send action. | A Gmail draft exists only after recorded approval; the operator can declare it sent or not sent. |
...
Purpose: approve a precise, respectful message without turning HORUS into a sending system.
...
- Demonstration URL, language proposal with evidence, source-data ages, and Gmail-draft status.

**Actions**

- Edit, return to demonstration, refresh evidence, approve to create Gmail draft, open the resulting draft, declare sent, or declare not sent.
...
- Prospect timeline with source retrievals, score versions, approvals, publication, Gmail-draft creation, declared send, engagement, and follow-up events.
...
| Decision | Decide whether to select, publish, or create a Gmail draft. | Factor/gate breakdown, observed indicators, completeness, freshness, source date, and blocking checklist. |
...
- [x] Outreach cannot proceed to Gmail drafting without a published URL, source-backed claims, freshness pass, language evidence/override record, and explicit approval.
...
| Language and Gmail draft-only outreach | Charter §§16–17; DEC-027, DEC-028 |
