...
- [x] Replay Finescape and Sons and SEASONS EATS in shadow mode and compare the agent output with retained evidence and historical operator decisions. (DEC-063, DEC-064; deterministic halves asserted in DEC-087)
- [ ] Add concept and outreach composition only after the analyst replay passes its evidence and missing-data checks.
- [x] Review security, prompt-injection resistance, privacy, subscription limits, product terms, and the handling of third-party business data. *Partly done — DEC-088 and [`SECURITY_REVIEW.md`](SECURITY_REVIEW.md) cover the agent boundary, the two network-reaching tools, prompt injection, and privacy. Subscription limits, product terms, a dependency audit, the unexercised publish/Gmail paths, and third-party data retention remain open, so the box stays unticked.* (DEC-088 — F4 remains open and is recorded as an accepted, tracked risk under DEC-099)
- [ ] Confirm minimum performance, repeatability, SerpApi consumption, and model-runtime availability per prospect. *Partly observed — DEC-092's first real run spent 10 credits for 1 search plus 3 scored candidates, and reproduced DEC-073's live figure to within 0.1. Model-runtime availability and repeatability across runs are still unmeasured, so the box stays unticked.*
