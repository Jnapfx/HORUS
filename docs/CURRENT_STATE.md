...
**Installed and building.** `npm install` pulled `@modelcontextprotocol/sdk` and `zod` cleanly, and `tsc -p tsconfig.electron.json` compiles `evidence-mcp-server.ts` against the real SDK types. 61 of 61 tests pass, including the one that opens a raw readonly SQLite handle and confirms a write throws. What remains: no Claude Code process has actually called `read_evidence_snapshot` over stdio yet — the MCP server has been built and typechecked, not run.
